...
if dpkg -l zenity python-apt python-gtk2 python-nose python-setuptools git-core python-vte &> /dev/null ; then
    echo "Require packages installed."
else
    echo "Require packages not installed."
    apt-get update
    apt-get -y --force-yes install zenity python-apt python-gtk2 git-core python-setuptools python-nose python-vte
fi
