...
if dpkg -l python-gtk2 python-nose python-setuptools git-core python-vte &> /dev/null ; then
    echo "Require packages installed."
else
    echo "Require packages not installed."
    apt-get update
    apt-get -y --force-yes install python-gtk2 git-core python-setuptools python-nose python-vte
fi
