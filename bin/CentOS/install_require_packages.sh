...
if rpm -q wget zenity python-nose python-setuptools vte git redhat-lsb &> /dev/null ; then
    echo "Require packages installed."
else
    echo "Require packages not installed."

    yum check-update
    yum -y install wget zenity git python-nose python-setuptools vte redhat-lsb
