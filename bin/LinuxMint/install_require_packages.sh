...
# which need by lazyscripts. This script is use for LinuxMint
...
if dpkg -l zenity python-apt python-nose python-setuptools git-core python-vte python-gtk2 &> /dev/null ; then
    echo "Require packages installed."
else
    echo "Require packages not installed."
    apt-get update
    apt-get -y --force-yes install zenity python-apt git-core python-setuptools python-nose python-vte python-gtk2
fi
